...
static void v_wifi_inf_handler( char *data, uint16_t len);
...
void app_main()
{
    ...
    set_wifi_inf_callback(v_wifi_inf_handler);
...
static void v_wifi_inf_handler( char *data, uint16_t len){
    ...
    /* config to station mode */
    if( (ssid != NULL) && (pass != NULL)){
        /* config */
        wifi_init_station();
    }
    else{
        ESP_LOGI(TAG, "please enter ssid and password\n");
    }
}
...
static void wifi_init_station( void ){
    ...
    /* test */
    count++;
    ESP_LOGI(TAG, "The event register count: %d\n", count);
